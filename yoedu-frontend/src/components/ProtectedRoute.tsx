...
export const RoleGuard: React.FC<RoleGuardProps> = ({ children, allowedRoles }) => {
  ...
  if (!allowedRoles.includes(user.role)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md">
        ...
        <p className="text-slate-400 max-w-md mb-6">
          Your account role (<span className="text-brand-400 font-semibold">{user.role}</span>) does not have permission to access this page.
        </p>
