...
class Day1ApplicationTests {
    ...
    @Test
    void testSearchCourseClasses() {
        System.out.println("========== TESTING SEARCH COURSE CLASSES ==========");
        try {
            org.springframework.data.domain.Page<com.yo.day1.dto.courseclass.CourseClassResponse> result = 
                courseClassService.search(null, null, null, null, org.springframework.data.domain.PageRequest.of(0, 10));
            System.out.println("Search result count: " + result.getTotalElements());
            result.getContent().forEach(c -> System.out.println("Result class: " + c.id() + " - " + c.name() + " - " + c.mainTeacherName()));
        } catch (Exception e) {
            System.err.println("SEARCH FAILED WITH EXCEPTION:");
            e.printStackTrace();
        }
    }
}
