  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Điểm danh Chuyên cần</h2>
          <p className="text-sm text-foreground-muted mt-1">Theo dõi và cập nhật tình trạng chuyên cần của học viên theo từng lớp học.</p>
        </div>
        <div className="bg-surface border border-border rounded-xl p-1.5 flex items-center gap-1 w-max">
           <button onClick={() => setActiveTab('daily')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'daily' ? 'bg-brand-600 text-white' : 'text-foreground-secondary hover:text-foreground hover:bg-surface-hover'}`}>Theo ngày</button>
           <button onClick={() => setActiveTab('matrix')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'matrix' ? 'bg-brand-600 text-white' : 'text-foreground-secondary hover:text-foreground hover:bg-surface-hover'}`}>Ma trận</button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-surface border border-border rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
             <div className="flex items-center gap-2 w-full md:w-auto">
                ...
                <input type="date" value={selectedDate} onChange={(e) => e.target.value && setSelectedDate(e.target.value)} className="bg-surface border border-border rounded-lg py-2 px-3 text-sm text-foreground focus:outline-none focus:border-brand-500" />
                <Button variant="secondary" onClick={() => shiftDate(1)}><ChevronRight size={16}/></Button>
                <Button variant="ghost" className="text-brand-600 dark:text-brand-400" onClick={setDateToday}>Hôm nay</Button>
             </div>
             <div className="flex items-center gap-4">
                <span className="text-sm font-medium text-brand-600 dark:text-brand-400">{formattedDateVietnamese}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             <div className="bg-surface border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="bg-surface-hover text-foreground-secondary p-3 rounded-lg"><BookOpen size={20}/></div>
                <div><div className="text-sm text-foreground-muted">Cần điểm danh</div><div className="text-xl font-bold text-foreground">{kpis.totalClasses} lớp</div></div>
             </div>
             <div className="bg-surface border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-3 rounded-lg"><CheckCircle size={20}/></div>
                <div><div className="text-sm text-foreground-muted">Đã điểm danh</div><div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{kpis.attendedClasses} lớp</div></div>
             </div>
             <div className="bg-surface border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="bg-amber-500/10 text-amber-600 dark:text-amber-400 p-3 rounded-lg"><Clock size={20}/></div>
                <div><div className="text-sm text-foreground-muted">Chưa điểm danh</div><div className="text-xl font-bold text-amber-600 dark:text-amber-400">{kpis.unattendedClasses} lớp</div></div>
             </div>
             <div className="bg-surface border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="bg-brand-500/10 text-brand-600 dark:text-brand-400 p-3 rounded-lg"><Percent size={20}/></div>
                <div><div className="text-sm text-foreground-muted">Tỉ lệ có mặt</div><div className="text-xl font-bold text-brand-600 dark:text-brand-400">{kpis.attendanceRate !== null ? `${kpis.attendanceRate}%` : '--'}</div></div>
             </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 bg-surface border border-border p-4 rounded-xl">
            <div className="relative w-full md:w-96">
              ...
              <input type="text" value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} placeholder="Tìm lớp, môn, giáo viên..." className="w-full bg-surface border border-border rounded-lg pl-10 pr-4 py-2 text-sm text-foreground placeholder-foreground-muted focus:outline-none focus:border-brand-500" />
            </div>
            <div className="flex items-center gap-2 overflow-x-auto">
               {['ALL', 'UNATTENDED', 'ATTENDED'].map(filter => (
                  <button key={filter} onClick={() => setStatusFilter(filter as any)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${statusFilter === filter ? 'bg-brand-500/10 text-brand-600 dark:text-brand-400 font-semibold' : 'text-foreground-secondary hover:bg-surface-hover'}`}>
                 ...
                 {isLoadingDailyClasses ? (
                    ...
                    filteredClasses.map(c => (
                       <TableRow key={c.classId}>
                          ...
                          <TableCell>
                             <div className="font-semibold text-foreground">{c.className}</div>
                             <div className="text-xs text-foreground-muted">{c.courseName}</div>
                          </TableCell>
                          <TableCell>
                             <div className="text-sm text-foreground-secondary">{c.scheduleLabel}</div>
                             <div className="text-xs text-foreground-muted">{c.roomName}</div>
                          </TableCell>
                          <TableCell className="text-foreground-secondary">{c.teacherName}</TableCell>
                          <TableCell className="text-center">
                             {c.isAttended ? (
                                <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">{c.presentCount} / {c.attendedCount}</div>
                             ) : (
                                <div className="text-xs text-foreground-muted">{c.totalStudents} HV</div>
                             )}
                          </TableCell>
                          <TableCell className="text-center">
             ...
             <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
                <thead>
                   <tr className="bg-surface-hover/50 border-b border-border">
                      <th className="py-3 px-4 font-semibold text-foreground sticky left-0 bg-surface z-10 w-48">Học viên</th>
                      {daysInMonth.map(d => <th key={d} className="py-3 px-2 text-center text-xs font-semibold text-foreground-muted min-w-[32px]">{d}</th>)}
                      <th className="py-3 px-4 text-center font-semibold text-emerald-600 dark:text-emerald-400">Có mặt</th>
                      <th className="py-3 px-4 text-center font-semibold text-rose-500">Vắng</th>
                      <th className="py-3 px-4 text-center font-semibold text-brand-600 dark:text-brand-400">Tỉ lệ</th>
                   </tr>
                </thead>
                <tbody className="divide-y divide-border text-foreground-secondary">
                   {isLoadingMatrix ? (
                      <tr><td colSpan={daysInMonth.length + 4} className="py-8 text-center text-foreground-muted">Đang tải...</td></tr>
                   ) : matrixData.length === 0 ? (
                      ...
                      matrixData.map(row => {
                         ...
                         return (
                            <tr key={row.studentId} className="hover:bg-surface-hover/40 transition-colors">
                               <td className="py-2 px-4 sticky left-0 bg-surface hover:bg-surface-hover/40 z-10 font-medium truncate max-w-[200px]" title={`${row.studentCode} - ${row.studentName}`}>
                                  {row.studentName}
                               </td>
                               {daysInMonth.map(d => {
                                  ...
                                  return (
                                     <td key={d} className="py-2 px-1 text-center font-mono text-xs">
                                        {!ds ? <span className="text-foreground-muted/40">-</span> : 
                                         ds.status === 'PRESENT' ? <span className="text-emerald-600 dark:text-emerald-400 font-bold">P</span> :
                                         ds.status === 'LATE' ? <span className="text-amber-500 font-bold">L</span> :
                                         ds.status === 'EXCUSED' ? <span className="text-blue-500 font-bold">E</span> :
                                         <span className="text-rose-500 font-bold">A</span>}
                                     </td>
                                  );
                               })}
                               <td className="py-2 px-4 text-center font-bold text-emerald-600 dark:text-emerald-400">{present}</td>
                               <td className="py-2 px-4 text-center font-bold text-rose-500">{absent + excused}</td>
                               <td className="py-2 px-4 text-center font-bold text-brand-600 dark:text-brand-400">{rate}%</td>
                            </tr>
                         );
                      })
                   )}
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={`Điểm danh: ${activeClass?.className}`} maxWidth="3xl">
         <div className="space-y-4">
            <div className="flex items-center justify-between bg-surface-hover/40 p-3 rounded-xl border border-border">
               <div className="text-sm text-foreground-secondary">
                  <span className="font-semibold text-foreground">{attendanceForm.length}</span> học viên
               </div>
            ...
            <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2">
               {attendanceForm.map((item, idx) => (
                  <div key={item.studentId} className="bg-surface border border-border p-3 rounded-xl flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                     <div className="flex items-center gap-3">
                        <span className="text-xs text-foreground-muted font-mono w-5">{idx + 1}</span>
                        <div>
                           <div className="font-semibold text-foreground">{item.fullName}</div>
                           <div className="text-xs text-brand-600 dark:text-brand-400 font-mono">{item.studentCode}</div>
                        </div>
                     </div>
                     <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-end sm:items-center">
                        <div className="flex bg-surface-hover border border-border rounded-lg p-1">
                           <button onClick={() => handleStatusChange(item.studentId, 'PRESENT')} className={`px-2 py-1 text-xs rounded-md font-medium ${item.status === 'PRESENT' ? 'bg-emerald-600 text-white' : 'text-foreground-secondary'}`}>Có mặt</button>
                           <button onClick={() => handleStatusChange(item.studentId, 'LATE')} className={`px-2 py-1 text-xs rounded-md font-medium ${item.status === 'LATE' ? 'bg-amber-500 text-slate-900' : 'text-foreground-secondary'}`}>Đi muộn</button>
                           <button onClick={() => handleStatusChange(item.studentId, 'EXCUSED')} className={`px-2 py-1 text-xs rounded-md font-medium ${item.status === 'EXCUSED' ? 'bg-blue-600 text-white' : 'text-foreground-secondary'}`}>Phép</button>
                           <button onClick={() => handleStatusChange(item.studentId, 'ABSENT')} className={`px-2 py-1 text-xs rounded-md font-medium ${item.status === 'ABSENT' ? 'bg-rose-600 text-white' : 'text-foreground-secondary'}`}>Vắng</button>
                        </div>
                        {(item.status !== 'PRESENT') && (
                           <input type="text" value={item.note} onChange={(e) => handleNoteChange(item.studentId, e.target.value)} placeholder="Ghi chú..." className="w-full sm:w-32 bg-surface border border-border rounded-md px-2 py-1 text-xs text-foreground focus:outline-none focus:border-brand-500" />
                        )}
                     </div>
                  </div>
               ))}
