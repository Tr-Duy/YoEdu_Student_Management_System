        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/50 text-sm text-slate-300">
              ...
              {isError && (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-red-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <AlertCircle size={28} className="text-red-400" />
                      <p className="font-semibold text-sm">{(error as any)?.message || 'Lỗi khi tải danh sách lớp học'}</p>
                      <button
                        type="button"
                        onClick={() => refetch()}
                        className="mt-2 text-xs bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2 rounded-xl transition-all cursor-pointer border border-slate-700"
                      >
                        Thử lại
                      </button>
                    </div>
              ...
              {!isLoading && !isError && classesList.map((cClass) => (
                <tr key={cClass.id} className="hover:bg-slate-900/25 transition-all duration-150">
                  ...
                  <td className="py-4 px-6 text-slate-300">
                    <div>
                      <span className="block text-slate-200 font-semibold">{cClass.courseName || 'Khóa học'}</span>
                  ...
                  <td className="py-4 px-6">
                    <div>
                      <span className="block text-slate-200 text-xs font-semibold flex items-center gap-1">
                        ...
                        {cClass.roomName || 'Chưa xếp phòng'}
                      </span>
                      <span className="block text-[11px] text-brand-300 font-bold flex items-center gap-1 mt-0.5">
                        ...
                        {cClass.scheduleLabel || 'Chưa xếp ca'}
                      </span>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-slate-400">
                    <div>
                      <span className="block text-slate-200 text-xs font-medium">{cClass.mainTeacherName || 'Chưa có GVCN'}</span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className="text-slate-200 font-bold block mt-0.5">{selectedClass.courseName || 'Khóa học'}</span>
                    </div>
                    <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className="text-slate-200 font-bold block mt-0.5 flex items-center gap-1.5">
                        ...
                        {selectedClass.roomName || 'Chưa xếp phòng'}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-slate-200 font-bold block mt-0.5 flex items-center gap-1.5">
                        ...
                        {selectedClass.scheduleLabel || 'Chưa xếp ca'}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-slate-200 font-bold block mt-0.5 flex items-center gap-1.5">
                        ...
                        {selectedClass.mainTeacherName || 'Chưa có GVCN'}
                      </span>
                    </div>
