  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-100">Ghi danh & Chuyển lớp</h2>
          <p className="text-sm text-slate-400 mt-1">Quản lý xếp lớp, điều chuyển học viên và theo dõi lịch sử học tập.</p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-2 flex items-center gap-2 w-max">
         <button onClick={() => setActiveTab('class')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'class' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>Tra cứu theo Lớp</button>
         <button onClick={() => setActiveTab('student')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'student' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>Tra cứu theo Học viên</button>
      </div>

      {activeTab === 'class' ? (
         <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
               <label className="block text-sm font-medium text-slate-400 mb-2">Chọn Lớp học để xem danh sách</label>
               <select
                 value={selectedClassId}
                 onChange={(e) => setSelectedClassId(e.target.value)}
                 className="w-full md:w-1/2 bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-slate-200 focus:outline-none focus:border-brand-500"
               >
                   ...
                   <TableBody>
                     {isClassEnrollmentsLoading ? (
                       ...
                       classEnrollments.map((e) => (
                         <TableRow key={e.id}>
                           <TableCell className="font-mono text-brand-400 font-medium">{e.studentCode}</TableCell>
                           <TableCell className="font-medium text-slate-200">{e.studentName}</TableCell>
                           <TableCell className="text-slate-400">{e.enrolledAt}</TableCell>
                           <TableCell className="font-semibold text-emerald-400">{formatVND(e.appliedFee || 0)}</TableCell>
                           <TableCell>
                           ...
                           <TableCell className="text-right">
                             <div className="flex justify-end gap-2">
                               ...
                               <Button variant="ghost" size="sm" className="text-rose-400 hover:text-rose-300" onClick={() => { setSelectedEnrollment(e); setIsConfirmDropOpen(true); }} title="Rút tên"><Trash2 size={16}/></Button>
                             </div>
                           </TableCell>
                         </TableRow>
                       ))
         ...
         <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
               <label className="block text-sm font-medium text-slate-400 mb-2">Chọn Học viên để xem lịch sử</label>
               <select
                   ...
                   <TableBody>
                     {isStudentEnrollmentsLoading ? (
                       ...
                       studentEnrollments.map((e) => (
                         <TableRow key={e.id}>
                           <TableCell className="font-mono text-brand-400 font-medium">{e.classCode}</TableCell>
                           <TableCell className="font-medium text-slate-200">{e.className}</TableCell>
                           <TableCell className="text-slate-400">{e.enrolledAt}</TableCell>
                           <TableCell>
      ...
      <Modal isOpen={isTransferOpen} onClose={() => setIsTransferOpen(false)} title="Chuyển lớp cho Học viên" maxWidth="lg">
        <form onSubmit={transferForm.handleSubmit(onTransferSubmit)} className="space-y-6">
          <div className="space-y-4">
            <div className="p-3 bg-slate-900 border border-slate-800 rounded-lg text-sm text-slate-300">
               <div className="flex justify-between mb-1"><span className="text-slate-500">Học viên:</span> <span className="font-semibold text-slate-200">{selectedEnrollment?.studentName} ({selectedEnrollment?.studentCode})</span></div>
               <div className="flex justify-between"><span className="text-slate-500">Lớp hiện tại:</span> <span className="font-semibold text-slate-200">{selectedEnrollment?.className} ({selectedEnrollment?.classCode})</span></div>
            </div>
